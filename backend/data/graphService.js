/**
 * graphService.js (Local Frontend Version)
 */
import { hanziData } from '../../data/hanziData';

export function buildGraph(maxHsk = 6, context = null, mode = 'evo', rootsOnly = false) {
...
export function expandNode(id, mode, maxHsk = 6) {
  let universe = hanziData.filter(n => n.hsk <= maxHsk);
  const activeIds = new Set(universe.map(n => n.id));
