...
export default function GraphCanvas() {
  ...
  return (
    <div className="flex-1 relative overflow-hidden bg-ink-950">
      ...
      {/* Loading overlay */}
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-ink-950/60 pointer-events-none">
          <div className="flex flex-col items-center gap-3">
            <div className="w-8 h-8 border-2 border-vermillion-500/40 border-t-vermillion-500 rounded-full animate-spin" />
            <span className="text-sm text-ink-400 font-mono">Carregando grafo...</span>
          </div>
        </div>
      )}
      ...
      {/* Error state */}
      {error && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-vermillion-500/20 border border-vermillion-500/40 text-vermillion-300 text-sm px-4 py-2 rounded-lg">
          {error}
        </div>
      )}
